...
// upgraded by Run applying exactly [2]. Every row survives with revision = 1
// and, because the new columns have constant defaults, no operations tuple is
// rewritten: xmin is unchanged for every operation row.
func TestRun_UpgradeFromVersion1KeepsRows(t *testing.T) {
	...
	if len(applied) != 1 || applied[0] != 2 {
		t.Fatalf("upgrade Run applied = %v, want [2]", applied)
	}

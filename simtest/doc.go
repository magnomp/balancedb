...
//     expected_revision guards, CONFIRMED / INVALID / already-edited / DELETED /
//     same-round PENDING targets), deletes (ADR-0011: single, grouped and mixed
//     with edits and new operations, the same target classes and guards),
//     state-aware limit changes, and idempotent replays/conflicts — through the
//     in-memory sequential reference model across 1,000+ seeds under `make test`,
//     asserting G1/G2/G4/G6, the editing invariants (edits never CONFIRMED, dense
//     append-only histories), the deletion invariants (a DELETED row has exactly
//     one APPLIED delete and never reads CONFIRMED again) and G3 reproducibility,
//     and printing the action mix (every edit and delete class must occur).
...
//     the reference model — statuses (DELETED included), current columns,
//     revisions, deletion stamps and the append-only operation_revisions rows,
//     transactions, balances, timelines and snapshots — interleaved with fault
//     injection: batch-boundary crashes (sim_db_test.go), competing-leader
//     failover, zombie-leader stale-lease writes, and the Guard-3 version-race
//     stressor (sim_faults_test.go), all with edits and deletes in the mix; the
//     ADR-0008 visibility schedules (registration_db_test.go) include
